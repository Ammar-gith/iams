                        ...
                        <div class="row">
                            ...
                            <div class="row">
                                <div class="col-md-8 lead-block">
                                    <p class="decorated-para">System auto-generated numerical series</p>
                                    <p class="lead-para">The INF Series is used to identify each Advertisement uniqly, typically begins with one &lpar;1&rpar;, incrementing for each Advertisement entering into the IAMS ecosystem.</p>
                                </div>
                                <div class="col-md-4 p-info-para">
                                    <p class="info-para">A new series initiates at the start of new year, automatically closing the previous one.</p>
                                </div>
                            </div>
                        </div>
                        <hr class="my-4" />
                                    ...
                                    <table class="table table-striped">
                                        <thead>
                                            <tr>
                                                ...
                                                <th scrop="col">Total Advertisements</th>
                                                <th scope="col">Started From</th>
                                                <th scope="col">Numbers Issued</th>
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>{{ sprintf('%05d-%s', $currentSeries->start_number, substr($currentSeries->series, -2)) }}</td>
                                                <td>{{ $currentSeries->issued_numbers }}</td>
                                                <td>{{ sprintf('%05d-%s', $currentSeries->start_number + $currentSeries->issued_numbers, substr($currentSeries->series, -2)) }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                @else
                                ...
                                <table class="table table-striped">
                                    ...
                                    <tbody>
                                        @foreach($previousSeries as $index => $series)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td>{{ sprintf('%05d-%s', $series->start_number, substr($series->series, -2)) }}</td>
                                                <td>{{ sprintf('%05d-%s', $series->start_number + $series->issued_numbers - 1, substr($series->series, -2)) }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                            @else
                                <p>No previous INF series found.</p>
                            @endif
                        </div>
                    </div>
                </div>
